...
.. figure:: https://img.shields.io/badge/-Python-fff?&logo=python
   :alt: Python

   Python

Aziona CLI is a cross CI platform tool that enables the building,
...
Aziona CLI is written in Python3 and can be used in various ways in both
local and pipelined environments, such as a:
...
The YML structure is divided into targets that include multiple stages;
...
**Modules** Modules are actions executed by a stage; they have different
types like Python or bash modules. Each module has to have defined the
type, module, and eventually additional arguments.
...
Create project folder, and create file ``.aziona.yml`` with the snippet
below.
...
This aziona configuration file firstly makes login on Ecr service,
nextly, build docker image, and endly push image to ecr repository
...
In the case in the local environment you want to use the aws profiles
configured in .aws/credentials you have to use the following
configuration:

1. You need to add this args ``--profile: ${AWS_PROFILE}`` to *login*
   stage
...
..
   ...
   [WARNING] It is not recommended to put sensitive data directly into
   the env.
...
Now that we’ve texted the process in a local environment, we can create
a pipeline in GitHub (or your reference system) that executes our
actions.

First, we need to create a “.github” folder in the root of the project
with another folder called “workflows” inside.
...
Then create a release.yml file in .github/workflows and paste the
following code:
...
This pipeline will be started whenever a push is made to the main. The
...
Create il file bitbucket-pipelines.yml nella root del progetto e
...
Create deployment named *env*. Enter the following keys in the
deployment:
...
v3.0 <https://github.com/azionaventures/aziona-cli/blob/main/LICENSE>`__

.. |image0| image:: https://img.shields.io/badge/version-0.1-green.svg
.. |image1| image:: https://img.shields.io/badge/docker--compose-build-blue.svg
.. |image2| image:: https://img.shields.io/badge/docker-build-blue.svg
